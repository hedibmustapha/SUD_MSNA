...
logbook<- makeslog(data,logbook,"id19",index,"uuid","duplicated uuid")
...
index<-pulluuid(data,data$child_absent>4)
logbook<- makeslog(data,logbook,"id11",index,"child_absent","Over 4 children away from home")
...
index<-pulluuid(data,data$difftime<10)
logbook<- makeslog(data,logbook,"id17",index,"difftime","Survey done in less than 10min")
...
index<-pulluuid(data,data$water_distance_mins>30&data$water_distance_mins%%5!=0)
logbook<- makeslog(data,logbook,"id20",index,"water_distance_mins","unusual number, too precise")

index<-pulluuid(data,data$water_distance_mins>199)
logbook<- makeslog(data,logbook,"id21",index,"water_distance_mins","it takes more than 199min to reach the water source")
